...
export function ResetPasswordForm() {
  ...
  return (
    <form className="space-y-4" onSubmit={submit}>
      <p className="text-sm text-text-subtle">Придумайте новый пароль для {email}</p>
      <div className="space-y-2.5">
        <label htmlFor="reset-password" className="block text-sm font-medium text-text-muted">
          Новый пароль
        </label>
      ...
      <div className="space-y-2.5">
        <label htmlFor="reset-password-confirm" className="block text-sm font-medium text-text-muted">
          Повторите пароль
        </label>
        <MenariumInput
        ...
        />
        {confirm ? (
          <p className={`text-xs ${password === confirm ? "text-accent" : "text-warning"}`} aria-live="polite">
