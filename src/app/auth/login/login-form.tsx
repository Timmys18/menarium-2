...
export function LoginForm() {
  ...
  return (
    <form className="space-y-5" onSubmit={submit}>
      ...
      {registered ? (
        <div className="rounded-control border border-teal-500/30 bg-teal-500/10 p-4 text-sm leading-6 text-accent" role="status">
      ...
      <div className="space-y-2.5">
        <div className="flex items-center justify-between gap-4">
          ...
          <Link
            ...
            className="inline-flex min-h-11 items-center rounded-lg px-2 text-sm text-accent transition hover:text-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)]"
          >
          Забыли пароль?
          </Link>
        </div>
      ...
      <div className="flex items-center gap-3 py-0.5" aria-hidden="true">
        <span className="h-px flex-1 bg-fill-3" />
        <span className="text-xs text-text-subtle">Первый раз здесь?</span>
        <span className="h-px flex-1 bg-fill-3" />
      </div>
      ...
      <p className="flex items-center justify-center gap-2 text-center text-xs text-text-subtle">
        <LockKeyhole className="h-3.5 w-3.5 text-accent-soft" />
