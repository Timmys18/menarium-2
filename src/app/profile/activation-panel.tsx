...
export function ActivationPanel({ activation }: { activation: ProfileActivation }) {
  ...
  return (
    <GlassCard className="relative overflow-hidden border border-line-hairline p-5 sm:p-6">
      <div
        className={cn(
          ...
          <span
            className={cn(
              ...
              nextAction.kind === "urgent"
                ? "bg-amber-300/12 text-warning"
                : "bg-teal-300/10 text-accent",
            )}
          >
          ...
          <div>
            <p
              className={cn(
                ...
                nextAction.kind === "urgent" ? "text-warning" : "text-accent",
              )}
            >
              {nextAction.eyebrow}
            </p>
            ...
            <p className="mt-1.5 max-w-2xl text-sm leading-5 text-text-subtle">
      ...
      {!activation.complete ? (
        <div className="relative mt-5 border-t border-line-hairline pt-4">
          <div className="mb-3 flex items-center justify-between gap-4">
            <p className="text-xs font-medium text-text-subtle">Путь до первого обмена</p>
                ...
                <li key={step.id} className="min-w-0">
                  <span
                    className={cn(
                      ...
                      step.done ? "bg-teal-400 text-on-accent" : "bg-fill-4 text-text-subtle",
                      step.done && "border-teal-300/40",
                      current && "border-line-strong bg-fill-4 text-text-primary",
                      !step.done && !current && "border-line-hairline",
                    )}
                  >
                  ...
                  <p
                    className={cn(
                      ...
                      current ? "font-semibold text-text-muted" : "text-text-subtle",
                    )}
                    title={step.label}
                  >
                    {step.label}
