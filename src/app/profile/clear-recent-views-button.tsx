  ...
  return (
    <span className="relative inline-flex w-full sm:w-auto">
      <button
        ...
        className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-control border border-white/10 bg-white/[0.03] px-3.5 text-xs font-semibold text-white/78 transition hover:border-rose-300/20 hover:bg-rose-300/[0.055] hover:text-rose-100/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] disabled:cursor-wait disabled:opacity-60 sm:w-auto"
      >
        {pending ? (
      ...
      {error ? (
        <span
          ...
          role="alert"
          className="absolute right-0 top-full z-20 mt-2 w-64 rounded-xs border border-rose-300/18 bg-[#151018] px-3 py-2 text-xs leading-5 text-rose-100/78 shadow-xl"
        >
          {error}
        </span>
      ) : null}
