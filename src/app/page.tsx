        ...
        */}
        <section className="mx-auto grid max-w-7xl gap-12 lg:min-h-[min(42rem,calc(100vh-12rem))] lg:grid-cols-[1.03fr_0.97fr] lg:items-center lg:gap-16">
          <div className="relative z-10">
            ...
            <p className="mt-7 max-w-2xl text-lg leading-8 text-text-subtle sm:text-xl">
            ...
            <div className="mt-8 flex flex-wrap gap-x-6 gap-y-3 text-sm text-text-subtle">
              {[
                "Вещи и услуги",
                "Любые сочетания",
                ...
                <span key={label} className="flex items-center gap-2">
                  <span className="flex h-5 w-5 items-center justify-center rounded-full bg-teal-300/10 text-accent">
          ...
          <div className="hero-canvas relative flex min-h-[420px] flex-col justify-center overflow-hidden rounded-panel p-5 sm:min-h-[480px] sm:p-8">
              ...
              */
              <div className="relative flex flex-col gap-4">
                {heroPair.map((item, index) => (
                  <Link
                    key={item.id}
                    href={`/item/${item.id}`}
                    className={`w-[82%] max-w-sm overflow-hidden rounded-card border border-line-default bg-[var(--surface-raised)] shadow-[0_24px_70px_rgba(0,0,0,0.42)] transition hover:z-30 hover:-translate-y-1 focus-visible:z-30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] ${
                  ...
                  >
                    ...
                    <div className="p-4 sm:p-5">
                      <p className="text-xs uppercase tracking-[0.14em] text-accent">{item.category}</p>
                      <h2 className="mt-1 line-clamp-1 text-base font-semibold sm:text-lg">{item.title}</h2>
                      <p className="mt-2 line-clamp-1 text-xs text-text-subtle">Ищу: {item.wanted}</p>
                    </div>
                  </Link>
                ))}
                <BrandMark
              ...
              <div className="relative flex min-h-[390px] items-center justify-center text-center">
                <div>
                  ...
                  <p className="mt-2 text-sm text-text-subtle">Добавьте вещь или услугу.</p>
                </div>
              </div>
            )}
          </div>
        ...
        <section className="mx-auto mt-28 max-w-7xl border-t border-line-hairline pt-20 md:mt-36 md:pt-24">
              ...
              {[
                ...
                return (
                  <SurfaceCard key={step.number} className="relative overflow-hidden p-6">
                    <div className="flex h-11 w-11 items-center justify-center rounded-control border border-line-default bg-fill-2 text-accent">
                      <Icon className="h-5 w-5" />
                    </div>
                    <h3 className="mt-6 text-lg font-semibold">{step.title}</h3>
                    <p className="mt-2 text-sm leading-6 text-text-subtle">{step.text}</p>
                  </SurfaceCard>
                );
              })}
            </div>
          ...
          <div className="mt-8 flex flex-wrap gap-3 text-xs text-text-subtle">
            <span className="inline-flex items-center gap-2 rounded-full border border-line-hairline bg-fill-1 px-3 py-2">
